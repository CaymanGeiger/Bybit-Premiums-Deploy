...
const CoinBorrowRates = ({ coinBorrowRates }) => {
  ...
  useEffect(() => {
    ...
    const storageWatchlist = JSON.parse(
      localStorage.getItem("borrow_watchlist")
    );
    if (storageWatchlist) {
  ...
  const sortedAndFilteredItems = sortedItems.filter((coinBorrowRate) => {
    return (
      coinBorrowRate.name !== undefined &&
      coinBorrowRate.name !== null &&
      coinBorrowRate.spotVolume !== undefined &&
      coinBorrowRate.spotVolume !== null &&
      coinBorrowRate.oneDayAverage !== undefined &&
      coinBorrowRate.oneDayAverage !== null &&
      coinBorrowRate.threeDayAverage !== undefined &&
      coinBorrowRate.threeDayAverage !== null &&
      coinBorrowRate.sevenDayAverage !== undefined &&
      coinBorrowRate.sevenDayAverage !== null &&
      coinBorrowRate.thirtyDayAverage !== undefined &&
      coinBorrowRate.thirtyDayAverage !== null &&
      coinBorrowRate.ninetyDayAverage !== undefined &&
      coinBorrowRate.ninetyDayAverage !== null
    );
  });
  ...
  return (
    <div className={styles.borrowMainDiv}>
      <div className={styles.headerDiv}>
        <h1 className={styles.borrowMainHeader}>BYBIT Borrow Rates</h1>
        <FontAwesomeIcon icon={faCircleInfo} className={styles.infoIcon} />
        <div className={styles.modalContent}>
          <p>
            The borrow rates are the interest rates that you pay when you borrow
            a cryptocurrency on Bybit.
          </p>
          <p>
            The rates are calculated based on the supply and demand of the
            cryptocurrency.
          </p>
          <p>The rates are updated every 2 hours.</p>
        </div>
      </div>
      {gettingData ? (
        <LoadingTable />
              ...
              <table className={`${styles.borrowTable} ${isStickyNameClicked}`}>
                ...
                <thead>
                  <tr>
                    ...
                    <th onClick={() => requestSort("spotVolume")}>
                      Spot Volume{" "}
                      <strong className={styles.arrows}>
                        {getSortIndicator("spotVolume")}
                      </strong>
                    </th>
                  ...
                  <AnimatePresence>
                    {finalItemsToDisplay.map((coinBorrowRate) => {
                      const handleCoinClick = (coinName) => {
                        ...
                        window.open(
                          `https://www.bybit.com/en/trade/spot/${coinName}/USDT?affiliate_id=62489`
                        );
                      };
                      const isInWatchlist = watchlist.includes(
                        coinBorrowRate.id
                      );
                      let isSymbol = coinBorrowRate.symbolUrl
                        ? coinBorrowRate.symbolUrl
                        : "/noImage.png";
                        ...
                        >
                          <td className={styles.tdSymbolAndName}>
                            <Image
                              ...
                              alt="coin symbol"
                              onClick={() =>
                                handleCoinClick(coinBorrowRate.name)
                              }
                            />
                            <span
                              onClick={() =>
                            ...
                            {isInWatchlist ? (
                              <h5
                                ...
                                onClick={() =>
                                  handleWatchlistChange(coinBorrowRate)
                                }
                              >
                                -
                              </h5>
